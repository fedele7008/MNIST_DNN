...
class TestDendrite():
    ...
    def test_dendrite_compile_exception_invalid_input_size(self, dendrite_4):
        ...
        with pytest.raises(ValueError):
            obj = dendrite_4.compile(input_size = -1, optimizer = 'gradient_descent', learning_rate = 0.001)
    ...
    def test_dendrite_forward(self, dendrite_4):
        ...
        dendrite_4.compile(input_size = 3, learning_rate = 0.001)
    ...
    def test_update_2_pass(self, dendrite_4):
        ...
        output_bias = np.array([0.4, 0.5, 0.6, 0.25])

        assert np.allclose(dendrite_4.weights, output_weights)
        assert np.allclose(dendrite_4.bias, output_bias)


    def test_dendrite_backward_exception_backward_not_initialized(self, dendrite_4):
        logging.debug(sys._getframe(0).f_code.co_name)
        logging.info('Testing Dendrite layer backward pass exception - backward not initialized')
