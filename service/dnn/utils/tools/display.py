...
"""
Display module is used to debug dataset, especially to display the image on
...
def show_img(image, is_normalized = True, display_mode = 'graphic'):
    ...
    if display_mode == 'ascii' or display_mode == 'simple-ascii':
        for row in range(28):
            line = ''
            for col in range(28):
                line += _GREYSCALE_MODE[display_mode][math.floor(img[row][col] 
                / 255 * (len(_GREYSCALE_MODE[display_mode]) - 1))]
            print(line)
    else:
